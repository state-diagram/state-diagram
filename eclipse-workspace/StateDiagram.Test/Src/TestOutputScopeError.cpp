...
TEST(OutputScopeErrorAuto)
{
  try
  {
    ...
    Auto state_TO_state{state, state, Output(is)};

    ASSERT(false);
  }
  catch (Transition::ScopeError::Output const & err)
  {
    ASSERT_EQ(err.scopePath, string() + "topState" + pathComponentSeparator + "region2" + pathComponentSeparator + "state");
...
TEST(OutputScopeErrorStep)
{
  try
  {
    ...
    Step state_TO_state{state, state, Output(is)};
