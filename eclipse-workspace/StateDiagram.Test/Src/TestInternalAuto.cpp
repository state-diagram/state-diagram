...
TEST(OutputScopeErrorOnInternalAuto)
{
  try
  {
    FSM_TOP(top);
    ...
    FSM_INTERNAL_AUTO(state, Output(local));
...
TEST(NestedInternalAuto)
{
  try
  {
    ...
    FSM_INIT(top);
    FSM_STATE(state, top);
    FSM_INTERNAL_AUTO(state, Action([&](){sawInternal = true;}), Max1Flag());
    FSM_FINAL(top);

    FSM_AUTO(top_INIT, state);
    FSM_STEP(state, top_FINAL, Trigger(global), Action([&](){sawStep = true;}));
